...
columns =  ['item_code','0','1']
#1. Fresh sold
fresh_sold = pd.read_csv('Fresh_Product_Sold.csv')
print fresh_sold.columns
fresh_sold.columns = columns
...
unsatisfied_fresh.columns = columns
...
unsatisfied_frozen.columns = columns
...
frozen_production.columns = columns
...
frozen_inventory.columns = columns
...
demand_fresh['0'] = 0
demand_fresh = demand_fresh.filter(items = ['item code','0',1])
demand_fresh.columns = columns
...
demand_frozen['0'] = 0
demand_frozen = demand_frozen.filter(items = ['item code','0',1])
demand_frozen.columns = columns
#print (demand_frozen)
...
df_final.columns = ['item_code', 'Day0', 'Day1', 'keyfigure']
