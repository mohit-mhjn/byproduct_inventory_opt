...
from pyomo.core.base import AbstractModel
from pyomo.environ import *
import pandas as pd
import os
# MIP formulation from A Mixed Integer Linear Program for Operational Planning Meat Planing Plant
# Vıctor M.Albornoz1, MarcelaGonzalez-Araya2, Matıas C.Gripe1 and SaraV.Rodrıguez3
# AVIOTA
# Rishikesh
model = ConcreteModel()
...
cut_pattern= list(i for i in range(1,len(y.index.get_level_values(level=1).unique())+1))
...
model.L = Set(initialize = L, ordered = True)     # shell life for fresh products
...
model.Ld = Set(initialize = Ld[:-1], ordered = True)
...
def objective_function(model):
    return sum(sum(model.pf[i]* model.vf[i,t]  + model.pc[i]* model.vc[i,t] for i in model.P) for t in model.T) - sum(sum(model.b*model.xc[i,t]  for i in model.P) for t in model.T) - sum(sum(model.hc[i] * model.Ic[i,t]  for i in model.P) for t in model.T) - sum(sum(sum(model.hf[i] * model.xf[i,t,t+l] for i in model.P) for t in model.T) for l in model.L)- sum(sum(model.sf[i]*model.uf[i,t] - model.sc[i]* model.uc[i,t] for i in model.P) for t in model.T) - sum(sum(sum(model.c[j]*model.z[j,r,t] + model.ce[j]*model.ze[j,r,t] for j in model.J) for r in model.R) for t in model.T) 
...
def fresh_frozne_balance(model,i,t):
        ...
        return model.x[i,t] == sum(model.xf[i,t,t+l] for l in model.Ld) + model.xc[i,t]
...
def fresh_product_wh_cap(model, t):
    return sum(sum(sum(model.xf[i,t-l,td] for td in range(t, t-l+len(L))) for l in model.L if t-l > 0)for i in model.P) <= model.Wf
